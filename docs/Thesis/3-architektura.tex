...
Preferowanie technologii \textit{cloud computing} jest powodowane przez wiele czynników, wœród których nale¿y wymieniæ ³atwoœæ dostêpu - potrzebna jest jedynie przegl¹darka. Z drugiej strony równie wa¿na jest ³atwoœæ zarz¹dzania - nie potrzeba etatu do doœwiadczonego administratora. Mo¿liwoœci chmur obliczeniowych s¹ docenianie tak¿e przez dostawców us³ug ze wzglêdu na ³atwe zarz¹dzanie infrastruktur¹, poniewa¿ centra danych posiadaj¹ jednorodny sprzêt i oprogramowanie, co wiêcej s¹ one pod kontrol¹ jednego kompetentnego podmiotu \cite{book:cloud2}.
...
zasobów. Klient rozliczany jest wiêc za moc obliczeniow¹ serwerów i czas ich pracy, iloœæ zapytañ do bazy, a tak¿e za bie¿¹ce wykorzystanie dysku. Dodatkowo architektura dynamicznie zwiêksza iloœæ zasobów, jak równie¿ instancji serwerów aktualnie
...
\begin{figure}[htbp]
\caption{Komponenty wchodz¹ce w sk³ad architektury w chmurze \cite{book:cloudComputingWiki}}
...
Jeœli skonfigurowano serwer zgodnie z rysunkiem \ref{fig:performance}, w zak³adce \textit{Instances} powinny zostaæ wyœwietlone wszystkie aktualnie wykorzystywane instancje serwerów. Dwie z nich to \textit{instancje sta³e} - rezydencji, wiêc ich czas pracy jest równy dobie. Dodatkowo, widoczne s¹ równie¿ \textit{instancje dynamiczne} (Rys. \ref{fig:performance2}), czyli takie, które u¿ywane s¹ doraŸnie. Zgodnie z ustawieniami, ³¹czna iloœæ instancji nie przekroczy górnego maksimum, czyli czterech serwerów.

W wypadku instancji dynamicznych nale¿y mieæ na uwadze, ¿e na op³atê sk³ada siê sumaryczny czas pracy oraz 15-minutowy czas op³aty rozruchowej. Dla przyk³adu, jeœli dynamiczna instancja pracuje przez piêæ minut, to koszt wynosi $0.08 / 60 \times  (15 + 5) = 2.6\cent$.
...
W zak³adce \textit{Instances} mo¿na równie¿ zobaczyæ takie statystyki, jak iloœæ operacji na minutê (\textit{Average QPS}), œrednie zu¿ycie pamiêci (\textit{Average Memory}) lub œredni czas przetwarzania ¿¹dania (\textit{Average Latency}). Dodatkowo, jeœli któraœ z instancji uleg³a przeci¹¿eniu, w skutek czego nie reaguje, to mo¿na j¹ zamkn¹æ rêcznie, klikaj¹c przycisk \textit{Shutdown}.
